...
namespace ATool
{
    /// <summary>
    /// 数据库大批量插入数据
    /// 引自：https://www.cnblogs.com/so9527/p/6193154.html
    /// </summary>
    public static class SqlConnectionExtension
    {
        ...
        public static DataTable ToSqlBulkCopyDataTable<TModel>(List<TModel> modelList, SqlConnection conn,
            string tableName)
        {
            ...
            for (int i = 0; i < columns.Count; i++)
            {
                ...
                if (mappingProp == null)
                    throw new Exception(string.Format("model 类型 '{0}'未定义与表 '{1}' 列名为 '{2}' 映射的属性", modelType.FullName,
        ...
        static List<SysColumn> GetTableColumns(SqlConnection sourceConn, string tableName)
        {
            string sql =
                string.Format(
                    "select syscolumns.name,syscolumns.colorder from syscolumns inner join sysobjects on syscolumns.id=sysobjects.id where sysobjects.xtype='U' and sysobjects.name='{0}' order by syscolumns.colid asc",
                    tableName);
